...
def winning_move(board,piece):
    ...
    # check for positively slope diagnols
    for c in range(COLUMN_COUNT - 3):
        for r in range(ROW_COUNT - 3):
...
def draw_board(board):
    ...
    for c in range(COLUMN_COUNT):
         for r in range(ROW_COUNT):
            if board[r][c] == 1:
                pygame.draw.circle(screen, MAROON, (int(c * SQUARESIZE + SQUARESIZE / 2), height -int(r * SQUARESIZE + SQUARESIZE / 2)), RADIUS)
            elif board[r][c] == 2:
...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            posx = event.pos[0]
            if turn == 0:
                pygame.draw.circle(screen, MAROON, (posx, int(SQUARESIZE / 2)), RADIUS)
            else:
           ...
           if turn == 0:
              ...
              if is_valid_location(board, col):
                 ...
                 if winning_move(board, 1):
                    label = myfont.render("Player 1 wins!!", 1, MAROON)
